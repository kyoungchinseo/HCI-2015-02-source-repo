...
void ofApp::draw(){
    ...
    for (int i = 0; i < numUsers; i++){
        ...
        msg += "Head: " +ofToString(headPos.x) +"," + ofToString(headPos.y)+
            ","+ofToString(headPos.z)+"\n";
        
        if ((headPos.y < leftHandPos.y) && (headPos.y < rightHandPos.y))
        {
            effectList[1].startEffect(headPos.x,headPos.y);
        }
        if (ofDist(leftHandPos.x, leftHandPos.y , rightHandPos.x, rightHandPos.y) < 50) {
            effectList[0].startEffect(headPos.x,headPos.y);
        }
    ...
    effectList[0].display();
    effectList[1].display();
